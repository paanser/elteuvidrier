...
if st.checkbox("Afegir cant polit"):
    metodo_canto = st.radio("¿Cómo deseas calcular el precio del canto pulido?",
                            ["Seleccionar desde tarifa MAPFRE", "Preu manual"])

    if metodo_canto == "Seleccionar desde tarifa MAPFRE":
        seleccion_canto = st.selectbox("Selecciona el tipo de canto pulido:", filtro_canto["Descripció"])
        precio_canto = filtro_canto.query("Descripció == @seleccion_canto")["Precio (€)"].values[0]
    else:
        precio_canto = st.number_input("Introdueix el preu per metre lineal (€):", min_value=0.0, step=0.1)
...
# EXEMPLE DE RESUM MOSTRAT

st.markdown("### **Mesures reals**")
st.write(f"**Amplada:** {ancho_real} cm")
st.write(f"**Alçada:** {alto_real} cm")
st.write(f"**m² reals:** {round((ancho_real/100) * (alto_real/100), 2)} m²")

st.markdown("### **Mesures corregides (múltiple de 6 segons taula)**")
st.write(f"**Amplada corregida:** {ancho_corr} cm")
st.write(f"**Alçada corregida:** {alto_corr} cm")
st.write(f"**m² corregits:** {area_corregida} m²")

preu_m2 = 45  # simulación
preu_sense_iva = round(area_corregida * preu_m2, 2)
iva = round(preu_sense_iva * 0.21, 2)
total = round(preu_sense_iva + iva, 2)

st.markdown("### **Preu**")
st.write(f"**Preu per m² (manual):** {preu_m2} €")
st.write(f"**Preu sense IVA:** {preu_sense_iva} €")
